...
(function(ns, window) {
    ...
    function initSDK(document) {
        ...
        console.log('[backend] initSDK Called', SDK);
        ...
        body.onload = function () {
        	...
        	//$("#waitingForm").show();
            window.initAsync();
            console.log('[backend] init', SDK.config.language);
        };
    }
    ...
    function callbackTask(data, successCallback, errorCallback) {
        console.log(" [backend] callbackTask called with timeout: " + data.timeout);
        setTimeout(function () {
    ...
    function OTP_Init(payload, config, successCallback, errorCallback) {
        console.log(" [backend] [OTP_Init] payload: ", payload);
        
        // 1) define command
    	var command_name = "otp";
        var window_name = command_name;
    	//console.log('[backend] [OTP_Init] successCallback', successCallback);
    	//console.log('[backend] [OTP_Init] errorCallback', errorCallback);
        ...
        var provider = null;
        // console.log('[backend] test', OPENED_WINDOWS["opts"]);
        ...
        if (provider && provider.match(".*ARUBA.*")) {
        	...
        	otp_form_url = "infocert_otp_form.html";
        }
    ...
    function OTP_Aruba_Init(payload, config, successCallback, errorCallback) {
        console.log(" [backend] [OTP_Aruba_Init] payload: ", payload);
        // 1) define command
        var command_name = "otp_aruba";
        window_name = "otp";
        // 2) register command's callbacks
        console.log('[backend] [OTP_Aruba_Init] - successCallback', successCallback);
        console.log('[backend] [OTP_Aruba_Init] - errorCallback', errorCallback);
        registerCallBacks(command_name, successCallback, errorCallback);
    ...
    function OTP_FEA_Init(payload, config, successCallback, errorCallback) {
        console.log(" [backend] [OTP_FEA_Init] payload: ", payload);
    ...
    function SIGNATURE_PlaceHolder_Init(payload, config, successCallback, errorCallback) {
                console.log(" [backend] [SIGNATURE_PlaceHolder_Init] payload: ", payload);
    ...
    function registerListeners( listeners ) {
        if ( listeners !== null && listeners !== undefined ) {
            if ( typeof listeners === 'object' ) {
                ...
                // do nothing.
                console.log(" [backend] [registerListeners] can't register listeners: ", listeners);
            }
        }
    ...
    function otp_callback(payload) {
        console.log(" [backend] [otp_callback] payload: " + JSON.stringify(payload));
        ...
        console.log('[backend] callbacks otp', callbacks);
        if (payload.op && payload.op == 'requestOTP') {
        	console.log('[backend] Request OTP');
        	var config = {
                headers : {
    ...
    function aruba_pin_callback(payload) {
        console.log(" [backend] [aruba_pin_callback] payload: " + JSON.stringify(payload));
        ...
        var callbacks = getCallbacksForTask(task, true);
        console.log('[backend] [aruba_pin_callback] callbacks', callbacks);
        ...
        if (json.code === 200) {
            ...
            if (!SDK.config.iframe || SDK.config.iframe == false) {
            	console.log('[backend] close window');
        		closeWindow("otp");
            }
            fn_requestArubaOTPAndSign(payload, SDK.config, callbacks.success, callbacks.error);
        }
        else {
        	console.log('[backend] sono qui e dovrei mostrare l\'errore');
    ...
    function fea_callback(payload) {
        console.log(" [backend] [fea_callback] payload: " + JSON.stringify(payload));
    ...
    function signature_ph_callback(payload) {
        console.log(" [backend] [signature_ph_callback] payload: " + JSON.stringify(payload));
    ...
    function fn_requestAndProcessOTP(payload, config, successCallback, errorCallback) {
        ...
        console.log(" [backend]  [fn_requestAndProcessOTP]: PROCESS_TOKEN: " + processToken);
        ...
        if (!payload.provider) {
	        ...
	        fn_doGet( SDK.config.CERT_OTP_URL + processToken, 
                ...
                function(xhr) { // SUCCESS CALLBACK
                    ...
                    console.log(" [backend] otpType: " + otpType);
                    ...
                    console.log(" [backend] ------- FEA provider");
                    ...
                    fn_requestFEAOTPAndSign(pin_payload, 
                            ...
                            function(resp) {    /* success callback: request OTP and open form to sign */
                                console.log('[backend] CallBack FEA_init', resp);
                                var payloadOTP = {
                    ...
                    console.log(" [backend] ------------------ INFOCERT provider or ARUBA MOBILE");
    ...
    function manageSMSAruba(config, provider, otpType, processToken, successCallback, errorCallback) {
        ...
        console.log(" [backend] ------- ARUBA provider");
        console.log(" [backend] ------------------ ARUBA SMS or ARUBACALL");
    ...
    function fn_requestPlaceHolder(payload, config, successCallback, errorCallback) {
        console.log('[backend] chiamata fn_requestPlaceHolder');
        ...
        */
        ...
        var processToken = requestPayload.token;
        console.log(" [backend]  [fn_requestPlaceHolder]: PROCESS_TOKEN: " + processToken);

        var fileId = requestPayload.fileId;
        console.log(" [backend]  [fn_requestPlaceHolder]: FILE_ID: " + fileId);
        ...
        fn_requestSignaturePlaceHolder(pin_payload, 
                config, 
                function(resp) {
                    console.log('[backend] CallBack SIGNATURE_PH_init', resp);
                    var payloadOTP = {
                            'process_token' : resp.token,
    ...
    function fn_requestFEAOTPAndSign(payload, config, successCallback, errorCallback) {
       console.log(" [backend] [fn_requestFEAOTPAndSign] payload: " + JSON.stringify(payload) );
        ...
        console.log(" [backend] fn_requestFEAOTPAndSign url:" +  requestedUrl);
    ...
    function fn_checkFEASignature(payload) {
        console.log(" [backend] [fn_checkFEASignature] payload: " + payload);
        return otp_sendResponse( payload );
        // fea_callback(payload);
    }
    
    function fn_checkPlaceHolderSignature(payload) {
        console.log(" [backend] [fn_checkPlaceHolderSignature] payload: " + payload);
        return otp_sendResponse( payload );
        // fea_callback(payload);
    }
    ...
    function fn_requestArubaOTPAndSign(payload, config, successCallback, errorCallback) {
       console.log(" [backend] [fn_requestArubaOTPAndSign] payload: " + JSON.stringify(payload) );
       console.log('[backend] fn_requestArubaOTPAndSign] callback', successCallback);
        ...
        // on success completion open OTP and PIN page submission
        console.log('[backend] send otp_payload',otp_payload);
        OTP_Init(otp_payload, otp_config, successCallback, errorCallback);
    ...
    function fn_init(confObj) {
    	console.log('[backend] fn_init', confObj);
    	console.log('[backend] fn_init', SDK);
    ...
    function messageHandler(event) {
        ...
        if ( !mustFilter(event.data) ) {
            ...
            if ( isACK( payload ) ) {
                ...
                console.log(" [backend] [messageHandler] received ACK for command message [" + payload.ACK + "]");
                clearInterval( timer );
                console.log(" [backend] [messageHandler] continuous sending for command message [" + payload.ACK + "] interrupted");
            }
            else {
                 var callbackName = payload.callback;

                 console.log(" [backend] [messageHandler]", payload);
    ...
    function registerCallBacks(task_name, successCallback, errorCallback) {
        ...
        console.log('[backend] CALLBACKS', JSON.stringify(REGISTERED_CALLBACKS));
    }
    ...
    function defaultSuccessCallback() {
        console.log(" [backend] defaultSuccessCallback: SUCCESS");
    }
    ...
    function defaultErrorCallback() {
        console.log(" [backend] defaultErrorCallback: ERROR");
    }
    ...
    /*
     ...
     console.log('[backend] [fn_doGet] url: ' + url );
     xhr.send();
     }
     ...
     console.log('[backend] [fn_doPost] url: ' + url + ', payload: ' + stringified_payload );
    ...
    function fn_doPut(url, payload, headers, successCallback, errorCallback) {
        ...
        console.log('[backend] [fn_doPut] url: ' + url + ', payload: ' + stringified_payload);
        // try {
    ...
    function fn_doDelete(url, payload, headers, successCallback, errorCallback) {
        var stringified_payload = processPayload(payload, headers);
        console.log('[backend] [fn_doDelete] url: ' + url + ', payload: ' + stringified_payload);
        // try {
        ...
        xhr.onerror = function () {
            if (!alreadyNotifiedError) {
                if (xhr.readyState === 4) {
                    ...
                    console.log(" [backend] STATUS " + xhr.status + ",  " + xhr.statusText);
                }
                    ...
                    console.log(" [backend] SOMETHING WEIRD HAPPENED");
                }
        ...
        if (( typeof( headers ) === 'object' ) && xmlHttpRequest) {
            ...
            for (header in headers) {
                if (headers.hasOwnProperty(header)) {
                    ...
                    console.log(" [backend] [setHeaders] " + header + " : " + value);
    ...
    function postMessage(win_ref, command_name, jsonArgs) {
       ...
       // setTimeout(function () {
           var timer = setInterval( function() {
                ...
                console.log(" [backend] [postMessage] " + stringified);
           }, timeoutInterval);
    ...
    function fn_processOTP(otp, pin) {
        ...
        // console.log(" [backend] OTP[" + otp + "], PIN[" + pin + "]");
        ...
        var headers = {
            ...
            "Authorization": SDK.OTP.commandArgs.auth_token
        };
    ...
    function fn_processArubaPin(pin, token) {
        console.log(" [backend] PIN[" + pin + "], TOKEN: " + token);
        ...
        console.log(" [backend] fn_requestArubaOTPAndSign url:" +  requestedUrl);
        ...
        fn_doPost(requestedUrl,
            ...
            headers,
            function (xhr) { // POST success callback
            	console.log('[backend] dovrei andare avanti');
            	var payload = {
                    "pin": pin,
                    "token" : token,
                    "code" : 200
            ...
            function(xhr) { // POST errorCallback
            	console.log('[backend] dovrei dare errore (' + SDK.config.provider + '\')');
    ...
    function otp_sendResponse( payload ) {
        console.log('[backend] [otp_sendResponse]', JSON.stringify(payload));
        if (payload && payload.data) {
        	console.log('[backend] payload.data', typeof payload.data);
            ...
            if (jsonData.description === 'CONFIRMA_ERROR_20' || jsonData.description === 'CONFIRMA_ERROR_20_ARUBA'
            	...
            	if (typeof submitError === 'function') {
            		console.log('[backend] mostro l\'errore (' + SDK.config.provider + '\')');
            		...
            		if (jsonData.description === 'CONFIRMA_ERROR_20' || jsonData.description === 'CONFIRMA_ERROR_22') {
            			...
            			console.log('[backend] errore', errore);
            			console.log('[backend] desc', jsonData.description);
            			if (!SDK.config.language) {
            				SDK.config.language = "EN";
            			}
            			console.log('[backend] lang', SDK.config.language);
            			submitError(errore);
            		...
            		console.log('[backend] ho idea che continui a non trovare submitError');
            	}
            } else {
    ...
    function otp_receivedMessage(event) {
        ...
        if ( !mustFilter( event.data )) {
            ...
            try {
                ...
                console.log(" [backend] [otp_sendResponse] error:", err);
                payload = null;
            }
            if( payload !== null && payload.command === 'otp') {
                OTP.messageSrc = event.source;
                OTP.commandArgs = payload.args; // extract command's args
                console.log(" [backend] [otp_sendResponse] OTP command received:" , OTP.commandArgs);
                ...
                console.log(" [backend] [otp_sendResponse] sent ACK for command " + payload.command);
            }
            else if ( payload !== null && payload.command === 'otp_aruba') {
                OTP.messageSrc = event.source;
                OTP.commandArgs = payload.args; // extract command's args
                console.log(" [backend] [otp_sendResponse] otp_aruba command received:" , OTP.commandArgs);
                if (OTP.commandArgs.language) {
                	SDK.config.language = OTP.commandArgs.language;
                }
                setLanguage(SDK.config.language);
                ...
                setLanguage(SDK.config.language);
                console.log(" [backend] [otp_sendResponse] fea command received:" , OTP.commandArgs);
                ...
                console.log('[backend] [signature_ph_receivedMessage]', JSON.stringify(event));
                OTP.messageSrc = event.source;
                OTP.commandArgs = payload.args; // extract command's args
                if (OTP.commandArgs.language) {
                	SDK.config.language = OTP.commandArgs.language;
                ...
                setLanguage(SDK.config.language);
                console.log(" [backend] [otp_sendResponse] signature_ph command received:" , OTP.commandArgs);
    ...
    function fea_receivedMessage(event) {
        ...
        if ( !mustFilter( event.data )) {
            ...
            try {
                ...
                console.log(" [backend] [fea_receivedMessage] error:", err);
                payload = null;
            }
            if ( payload !== null && payload.command === 'fea') {
                OTP.messageSrc = event.source;
                OTP.commandArgs = payload.args; // extract command's args
                console.log(" [backend] [fea_receivedMessage] fea command received:" , OTP.commandArgs);

                // send ACK to sender to suspend continuous delivery of "otp" command message;
                var ack = { "ACK" : payload.command };
                otp_sendResponse( ack );
                console.log(" [backend] [fea_receivedMessage] sent ACK for command " + payload.command);
            }
        }
    };
    ...
    function signature_ph_receivedMessage(event) {
        ...
        if ( !mustFilter( event.data )) {
            ...
            if ( payload !== null && payload.command === 'signature_ph') {
                OTP.messageSrc = event.source;
                OTP.commandArgs = payload.args; // extract command's args
                console.log(" [backend] [signature_ph_receivedMessage] fea command received:" , OTP.commandArgs);

                // send ACK to sender to suspend continuous delivery of "otp" command message;
                var ack = { "ACK" : payload.command };
                otp_sendResponse( ack );
                console.log(" [backend] [signature_ph_receivedMessage] sent ACK for command " + payload.command);
            }
        }
    };

    function mustFilter(data) {
        console.log(" [backend] [mustFilter]: '" + data + "'");
